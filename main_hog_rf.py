...
for image in data_gray:
    fd, hog_image = hog(image, orientations=orientation, pixels_per_cell=(
            ppc, ppc), cells_per_block=(cell_per_block, cell_per_block), block_norm='L2', visualize=True)
    hog_images.append(hog_image)
    # fd = scalify.fit_transform(fd)
...
hog_features = scalify.fit_transform(hog_features)
...
save_pickle(clf, 'best_model.pickle.gz')
...
for row in range(0, height, STEP_SIZE):
    for col in range(0, width, STEP_SIZE):
        ...
        fd, hog_image = hog(area, orientations=16,t
